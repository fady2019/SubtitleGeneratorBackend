...
from validation.subtitles import subtitle_begin_generation_audio_validator, subtitle_begin_generation_title_validator
...
@validate_token()
@input_validator(input_source=RequestFiles(), validator=subtitle_begin_generation_audio_validator)
@input_validator(input_source=RequestForm(), validator=subtitle_begin_generation_title_validator)
...
def begin_subtitle_generation():
    ...
    data = subtitles_service.generate_subtitle(
        user_id, {"title": request.form.get("title"), "audio": request.files.get("audio")}
    )
